...
abstract class AbstractAdminPage
{
    ...
    protected function locationImageThumbnailMarkup(int $attachmentId, string $label): string
    {
        if ($attachmentId <= 0) {
            return '<span class="eim-location-image-empty" aria-hidden="true">—</span>';
        }
        ...
        return sprintf(
            '<button type="button" class="button-link eim-location-image-thumb" data-full-src="%s" data-caption="%s" aria-label="%s"><img src="%s" alt="" loading="lazy"></button>',
            esc_url($fullUrl),
            esc_attr($label),
    ...
    protected function renderNotice(string $messageKey, string $errorKey = '', int $count = 0): void
    {
        $successes = [
            ...
            'budget_item_created'        => 'Line item added to library.',
            'budget_item_updated'        => 'Line item updated.',
            'budget_item_deleted'        => 'Line item deleted from library.',
        ...
        $errors = [
            ...
            'invite_no_email'            => 'This invitee does not have an email address on file, so no invite email was sent.',
            ...
            'budget_item_label_required' => 'A label is required to save a line item.',
            'budget_item_save_failed'    => 'Could not save the line item. Please try again.',
